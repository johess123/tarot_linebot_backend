...
async def insert_qes_ans(qes: str, qes_embedding, ans: str, ans_embedding):
    db = await get_db()
...
async def update_qes_ans(serial_number: int, qes: str, qes_embedding, ans: str, ans_embedding):
    db = await get_db()
    question_update = {
        "$set": {
            "content": qes,
